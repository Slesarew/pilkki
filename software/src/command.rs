...
use crate::{ecc::calculate_crc, serial::{communicate, Port, read_data, write_chunk}};
...
pub fn erase(port: &mut Port, addr: u32, pages: Option<u32>) {    
    ...
    assert!(page_count <= max_pages, "Impossible to erase {page_count} pages. Maximum {max_pages}");
    ...
    let buf_size_words = buf_size_str.strip_prefix(b"MaxBuf = ").expect("Failed to get buffer size");
    let buf_size_words: u32 = String::from_utf8(buf_size_words[..4].to_vec()).unwrap().parse().unwrap();
...
pub fn read_crc(port: &mut Port, addr: u32, len: Option<u32>) -> u32 {
    ...
    let mut crc_str = String::from_utf8(crc_str.strip_prefix(b"Crc32 = 0x").expect("Failed to get CRC").to_vec()).unwrap();
...
pub fn write_c(port: &mut Port, mut data: Vec<u8>, addr: u32, len: Option<u32>) {
    ...
    let info = chop(&info_str);
    let flash_size: u32 = info.get("FlashSize").unwrap().parse().unwrap();
    assert!(flash_size >= len, "Firmware would not fit. Available {flash_size} bytes, needed {len} bytes.");
    let page_size: u32 = info.get("PageSize").unwrap().parse().unwrap();
    ...
    for (i, chunk) in data.chunks(buf_size_bytes as usize).enumerate() {
        println!("Writing {} bytes at 0x{:08x}", chunk.len(), addr + (i as u32)*buf_size_bytes);
        let r = communicate(port, &cmd(&format!("loadbuffer {}", chunk.len()/4)));
        r.strip_prefix(b"Load ready").expect("loadbuffer - operation failed");

        write_chunk(port, chunk);
        let c = format!("writebuffer {} {}", addr + (i as u32)*buf_size_bytes, chunk.len()/4);
